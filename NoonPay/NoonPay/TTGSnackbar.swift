...
open class TTGSnackbar: UIView {
    ...
    /// Snackbar min height
    fileprivate static let snackbarMinHeight: CGFloat = 50
    
    /// Snackbar icon imageView default width
    fileprivate static let snackbarIconImageViewWidth: CGFloat = 16
    ...
    /// Tabbar Height
    @objc open dynamic var tabbarHeight : CGFloat = 0
    ...
    @objc open dynamic var bottomMargin: CGFloat = 0 {
        didSet {
            bottomMarginConstraint?.constant = -bottomMargin - tabbarHeight
            superview?.layoutIfNeeded()
        }
    }
    ...
    @objc open dynamic var contentInset: UIEdgeInsets = UIEdgeInsets.init(top: 0, left: 16, bottom: 0, right: 16) {
        didSet {
...
public extension TTGSnackbar {
        ...
        if let superView = containerView ?? UIApplication.shared.delegate?.window ?? UIApplication.shared.keyWindow {
            ...
            if #available(iOS 11.0, *) {
                bottomMarginConstraint = NSLayoutConstraint.init(
                    ...
                    toItem: superView.safeAreaLayoutGuide, attribute: .bottom, multiplier: 1, constant: -bottomMargin - tabbarHeight)
            } else {
                bottomMarginConstraint = NSLayoutConstraint.init(
                    item: self, attribute: .bottom, relatedBy: .equal,
                    toItem: superView, attribute: .bottom, multiplier: 1, constant: -bottomMargin - tabbarHeight)
            }
    ...
    fileprivate func showWithAnimation() {
        ...
        case .slideFromBottomBackToBottom, .slideFromBottomToTop:
            bottomMarginConstraint?.constant = snackbarHeight + tabbarHeight
...
private extension TTGSnackbar {
    ...
    func configure() {
        ...
        let hConstraints = NSLayoutConstraint.constraints(
            withVisualFormat: "H:|-0-[iconImageView]-8-[messageLabel]-2-[seperateView(0.5)]-2-[actionButton(>=44@999)]-0-[secondActionButton(>=44@999)]-0-|",
